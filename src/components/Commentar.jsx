      ...
      <div className="flex-grow min-w-0">
        <div className="flex items-center justify-between gap-4 mb-2">
          <div className="flex items-center gap-2">
            <h4 className={`font-medium truncate ${isPinned ? "text-indigo-200" : "text-white"}`}>{comment.user_name}</h4>
            {isPinned && <span className="px-2 py-0.5 text-xs bg-indigo-500/20 text-indigo-300 rounded-full">Admin</span>}
          </div>
...
const Komentar = () => {
  ...
  return (
    <div className="w-full bg-gradient-to-b from-white/30 via-white/10 to-pink-50/5 rounded-2xl backdrop-blur-xl shadow-xl border border-pink-100/20" data-aos="fade-up" data-aos-duration="1000">
      {/* Header */}
      <div className="p-6 border-b border-pink-200/20" data-aos="fade-down" data-aos-duration="800">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-xl bg-pink-200/20">
            <MessageCircle className="w-6 h-6 text-pink-400" />
          </div>
          <h3 className="text-xl font-semibold text-gray-800">
            Comments <span className="text-pink-500 font-medium">({totalComments})</span>
          </h3>
        </div>
      </div>
      ...
      <div className="p-6 space-y-6">
        {/* Error message */}
        {error && (
          <div className="flex items-center gap-2 p-4 text-rose-600 bg-rose-100/60 border border-rose-300/40 rounded-xl" data-aos="fade-in">
        ...
        <div className="space-y-4 h-[328px] overflow-y-auto overflow-x-hidden custom-scrollbar pt-1 pr-1" data-aos="fade-up" data-aos-delay="200">
          ...
          {comments.length === 0 && !pinnedComment ? (
            <div className="text-center py-8" data-aos="fade-in">
              <UserCircle2 className="w-12 h-12 text-pink-400 mx-auto mb-3 opacity-50" />
              <p className="text-gray-500">No comments yet. Start the conversation!</p>
            </div>
          ) : (
            comments.map((comment, index) => <Comment key={comment.id} comment={comment} formatDate={formatDate} index={index + (pinnedComment ? 1 : 0)} isPinned={false} />)
          )}
        </div>
      </div>
